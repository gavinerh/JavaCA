...
public class AdminLeaveController {
	...
	public String createNewLeave(@ModelAttribute("leave") @Valid Leave leave, BindingResult result, Model model,
			HttpSession ses, SessionStatus status) {
		if (!sess.isLoggedIn(ses, status))
			return "redirect:/";
		User u = user(ses);
		ArrayList<LeaveBalance> lb = lbService.findByUser(u);
	...
	public String editLeave(@ModelAttribute @Valid LeaveBalance lb, BindingResult result, HttpSession ses,
